...
void *main_thread_entry(void *arg0)
{
	...
	for (;;) {
		...
		/*
		 * Read measured telemetry and pack the 4-byte satellite
		 * payload: battery supply voltage (mV) + die temperature
		 * (whole degrees C). The packed bytes are logged so a serial
...
int main()
{
	...
	/*
	 * Battery Monitor and Temperature drivers share the on-chip BATMON
	 * peripheral. Both inits are idempotent and must run before the first
	 * measurement.
	 */
	BatteryMonitor_init();
	Temperature_init();
